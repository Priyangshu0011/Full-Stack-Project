...
<body>
  ...
  window.addEventListener("message", function (event) {
    ...
    if (!crop || !location) {
      aiResponseEl.innerHTML = '<span class="text-red-500">Please enter both crop and location.</span>';
      return;
    }
    ...
    const prompt = `
  You are an expert in smart irrigation systems. Give a recommendation for growing "${crop}" in "${location}".
  Keep the response within **100 words**. 
  Highlight key actions, terms, or advice in **bold** using markdown-style.
  ...
  `;
    ...
    try {
      ...
      if (data && data.candidates && data.candidates.length > 0) {
        ...
        // Convert **bold** markdown to <strong> for HTML display
        const formatted = rawText.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
        aiResponseEl.innerHTML = formatted;
      } else {
        aiResponseEl.innerHTML = '<span class="text-red-500">No response from AI. Try again later.</span>';
      }
    } catch (error) {
      console.error("AI Request Error:", error);
      aiResponseEl.innerHTML = '<span class="text-red-500">Failed to get recommendation. Check API key or network.</span>';
    }
  }
  </script>
</body>
